...
from keras.layers import Recurrent, Reshape
from keras.layers import Conv2D, UpSampling2D, MaxPooling2D, ConvLSTM2D, GaussianNoise
from keras.engine import InputSpec
from ..keras_utils import legacy_prednet_support


class PredNetParams(JSONEncoder):

    def __init__(self, stack_sizes, R_stack_sizes, A_filt_sizes, Ahat_filt_sizes, R_filt_sizes,
                 extrap_start_time=None, extrap_end_time=None, output_mode='error', **kwargs):
...
class PredNet(Recurrent):
    ...
    def build_from_params(params):
        return PredNet(params.stack_sizes, params.R_stack_sizes, params.A_filt_sizes, params.Ahat_filt_sizes,
                       params.R_filt_sizes, extrap_start_time=params.extrap_start_time,
                       extrap_end_time=params.extrap_end_time, output_mode=params.output_mode, **(params.args))


    @legacy_prednet_support
    def __init__(self, stack_sizes, R_stack_sizes, A_filt_sizes, Ahat_filt_sizes, R_filt_sizes, pixel_max=1.,
                 error_activation='relu', A_activation='relu', LSTM_activation='tanh',
                 LSTM_inner_activation='hard_sigmoid', output_mode='error', extrap_start_time=None,
                 extrap_end_time=None, data_format=K.image_data_format(), **kwargs):
    ...
    def get_initial_state(self, x):
        ...
        for u in states_to_pass:
            for l in range(nlayers_to_pass[u]):
                ds_factor = 2 ** l  # downsampling factor due to maxpooling
    ...
    def build(self, input_shape):
        ...
        if self.extrap_start_time is not None:
            ...
            if self.extrap_end_time == None:
                self.extrap_end_time = input_shape[1]  # if not specified extrap_end_time is the last frame
        ...
        if self.output_layer_type is None:
            if self.output_mode == 'prediction':
                ...
                for l in range(self.nb_layers):
                    ...
                    # use MSE wrt ground-truth as loss function when predicting N timesteps ahead
                    # if self.extrap_start_time is not None and l == 0:
                    #   squared_difference = tf.squared_difference(ground_truth, frame_prediction)
                    #   mse = K.mean(squared_difference, axis=[1,2])
                    #   layer_error = K.switch(t >= self.t_extrap_start and t < self.t_extrap_end, mse, layer_error)
                    all_error = layer_error if l == 0 else K.concatenate((all_error, layer_error), axis=-1)
                if self.output_mode == 'error':
                    output = all_error
                else:
                    # flattened_size = np.prod(frame_prediction.shape.as_list()[1:])
                    # flattened_prediction = Reshape((flattened_size,), name='flatten_prediction')(frame_prediction)
                    # output = K.concatenate((flattened_prediction, all_error), axis=-1)
